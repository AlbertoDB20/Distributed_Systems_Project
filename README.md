...
common/                      funzioni condivise fra le fasi e script di validazione
  ...
  aggiorna_ci.m                aggiornamento EKF in Covariance Intersection (§2.3)
  ...
  verifica_ci.m                validazione della CI, consistenza sotto correlazione ignota
...
theory/                      note teoriche di approfondimento
  ...
  TEORIA_CI.pdf                     Covariance Intersection: consistenza e scelta del peso
fase_1/ ... fase_5/         una cartella per fase, ciascuna con:
  mainN.m                      script di simulazione
  READMEN.md                   documentazione della fase
  risultati/                   figure generate dallo script
  genera_ambiente.m            (dalla fase 3) mappa, zone cieche, ancore UWB
...
| Distanze inter-veicolari nominali | $d_{ij}$ | 36-40 m (fino a 80 m dalla Fase 4) | formazione a "V" |
| Soglia di sicurezza | $d_{safe}$ | 15 m | ingombro fisico + margine |
| Raggio di comunicazione inter-veicolare | $r_{collab}$ | 120 m, **55 m dalla Fase 4** | vedi §3.3 |
...
Il peso $\gamma$ è scelto a ogni fusione minimizzando la traccia del blocco di posizione della $\Sigma$ risultante — problema convesso e scalare su $[0,1]$, risolto per sezione aurea. La consistenza vale però per **qualunque** $\gamma$: la minimizzazione sceglie il migliore fra risultati tutti garantiti, non è ciò che fornisce la garanzia.

> **Notazione.** Il documento di teoria [theory/TEORIA_CI.pdf](theory/TEORIA_CI.pdf) chiama $\omega$ questo peso. Nel progetto si usa $\gamma$, per non collidere con la velocità angolare $\omega$ del modello uniciclo.

**La CI classica non si applica direttamente** a questo caso: fonde due stime della *stessa* grandezza, mentre il vicino trasmette una stima del *proprio* stato, e da una misura di sola distanza non si ricava una stima puntuale della propria posizione ma una circonferenza. La formulazione corretta è quella di Carrillo-Arce et al. (IROS 2013): l'incertezza del vicino entra come rumore di misura aggiuntivo proiettato sulla congiungente, $R_{eff} = \sigma_{collab}^2 + u^T\Sigma_j^{(1:2,1:2)}u$, e poiché quel rumore è correlato in modo ignoto con il proprio prior, l'aggiornamento va eseguito in forma CI anziché con il guadagno di Kalman standard.

**Solo l'aggiornamento collaborativo richiede la CI.** GPS, AHRS, encoder e ancore fisse sono genuinamente indipendenti dalle stime dei vicini e restano su guadagno di Kalman standard: l'aggiornamento si spezza quindi in due blocchi in cascata, architettura nota come *Split Covariance Intersection*. Il blocco CI linearizza attorno al risultato del primo.

> **Stato di implementazione.** La CI è **implementata dalla Fase 5** (`common/aggiorna_ci.m`), insieme all'estensione del pacchetto scambiato da $\hat p_j$ a $(\hat p_j, \Sigma_j^{(1:2,1:2)})$. I due interventi vanno introdotti insieme, mai in due tempi: usare $R_{eff}$ con il guadagno di Kalman standard tratterebbe l'incertezza del vicino come rumore *indipendente*, cioè affermerebbe $P_{ij} = 0$ proprio nel punto in cui la correlazione nasce, ed è il doppio conteggio del Cap. 15 travestito da rigore. Fino alla Fase 4 il pacchetto conteneva quindi la sola posizione stimata, e il filtro era ottimista: limite noto e documentato — vedi [fase_3/README3.md §3.3](fase_3/README3.md) e [theory/TEORIA_osservabilita_e_filtro.md §1.4](theory/TEORIA_osservabilita_e_filtro.md), Caso E, per il legame con l'*observability mismatch*.

**La CI è un aggiornamento di Kalman con a priori e misura sgonfiati.** Posto $\Sigma_\gamma = \bar\Sigma/\gamma$ e $R_\gamma = R_{eff}/(1-\gamma)$, la definizione diventa $\Sigma^{-1} = \Sigma_\gamma^{-1} + C^T R_\gamma^{-1}C$, cioè la forma informativa dell'aggiornamento di Kalman. Ne segue che la CI si innesta su un EKF senza riscriverne l'architettura, e che valgono due proprietà: per $\gamma = 1$ la misura è ignorata e la fusione non degrada mai la stima; per $\gamma \to 0$ si butta via l'a priori e $\Sigma$ esplode nelle direzioni che la misura non vincola.

**Il prezzo della robustezza è una soglia di accoglimento.** Con a priori isotropo di varianza $s$ e misura scalare di varianza $R$ lungo una direzione, la derivata del costo in $\gamma = 1$ vale $s^2/R - 2s$: l'ottimo lascia il bordo, cioè la misura viene effettivamente usata, solo se

$$\mathrm{dev}(\text{a priori}) > \sqrt2\;\mathrm{dev}(\text{misura})$$

Una misura appena peggiore dell'a priori viene quindi **scartata del tutto**, non semplicemente pesata poco. È una conseguenza diretta della rinuncia all'ottimalità, ed è ciò che si osserva in Fase 5, dove in zona cieca l'a priori sostenuto da 4.5 ancore fisse è 7.3 volte migliore del ranging inter-veicolare e la CI lo rifiuta in tutte le fusioni ([fase_5/README5.md §1.5](fase_5/README5.md)).

**Validazione** (`common/verifica_ci.m`): su 20 000 prove Monte Carlo con due stime correlate al 74% in modo ignoto agli stimatori, il guadagno di Kalman standard dichiara 0.170 m² contro un errore reale di 0.297 m² (NEES 3.46, ottimista di 1.75×), mentre la CI dichiara 0.340 m² contro 0.310 m² reali (NEES 1.81, conservativa). La soglia $\sqrt2$ e l'equivalenza con la forma informativa sono verificate a precisione di macchina.
...
| Cicli $q$ richiesti | 1 | 49 su 50 |
| D-WLS contro WLS centralizzato | $1.5\cdot10^{-15}$ | $5.2\cdot10^{-7}$ |
| Guadagno su $\mathrm{dev}(c_{terr})$ per gli Slave | 8.1× e 8.3× | da **7.0×** a **15.1×** |

> **Valore contro molteplicità.** $\lambda_1(Q) = 1$ vale *sempre*, perché $Q$ è stocastica: è una garanzia strutturale, non una condizione da verificare. L'informazione sulla connettività sta nella sua **molteplicità**, $\mathrm{mol}_{\lambda_1}(Q)$, pari al numero di componenti connesse. Gli autovalori di $Q$ sono ordinati per **modulo** decrescente, il che rende $\rho_2 = \lvert\lambda_2(Q)\rvert$ vero per costruzione; in cambio $\lambda_n(Q)$ non è più il più negativo, e il test sulle oscillazioni va scritto su $\lambda_{min}(Q)$.
...
| $\mathrm{mol}_{\lambda_1}(L) = \mathrm{mol}_{\lambda_1}(Q)$ | numero di componenti connesse | 1 |
| $\rho_2 = \lvert\lambda_2(Q)\rvert$ | essential spectral radius: fattore di convergenza asintotico | 0.0000 |
| $\tau = 1/(K_{cons}\lambda_2(L))$ | costante di tempo dell'errore di formazione | 2.22 s |
...
| Fasi 4 e 5 | **55 m** | 5 archi su 10 | **0.697** | **0.826** |

Fino alla Fase 3 il raggio supera di tre volte l'estensione della formazione e il grafo è completo per costruzione: l'analisi spettrale è corretta ma inerte. Dalla **Fase 4** il raggio scende a 55 m — coerentemente con la fisica dell'antenna già argomentata in §2.2 — e cinque coppie su dieci non si vedono più. Le distanze nominali si separano in due gruppi netti, 36-40 m e 67-80 m, con un vuoto di 27 m in mezzo: la topologia resta deterministica con un margine del +35% sui link attivi e del −17% su quelli assenti.
...
* **Fase 5: Fusione Consistente e Realismo Off-Highway.** L'ordine interno è invertito rispetto alla prima stesura: la Covariance Intersection viene per prima, pur essendo indipendente dal modello di slittamento. Il motivo è che modifica il **formato del pacchetto scambiato**, e conviene farlo una volta sola; inoltre è il prerequisito perché l'incertezza variabile introdotta dallo slittamento venga poi trattata correttamente anziché aggravare un filtro già ottimista.
  1. **Contenuto dei pacchetti e Covariance Intersection** (*fatto*). Il pacchetto passa dalla sola $\hat p_j$ alla coppia $(\hat p_j,\ \Sigma_j^{(1:2,1:2)})$: **da 2 a 5 numeri**, cioè da 16 a 40 byte, pari a 800 B/s ricevuti per veicolo a 10 Hz con due vicini. L'aggiornamento si spezza in due — Kalman standard su AHRS, encoder, GNSS e ancore fisse; CI sul solo ranging inter-veicolare (§2.3) — e l'incertezza del vicino entra proiettata sulla congiungente.

     **Effetto misurato:** l'errore in zona cieca non cambia (0.077–0.087 m), mentre la covarianza dichiarata sale del 14% e il NEES di posizione si assesta fra 1.08 e 1.49 contro un valore atteso di 2. La CI non migliora la stima: toglie una confidenza che non era giustificata.

     **Risultato non previsto:** il peso $\gamma$ torna pari a 1 in **tutte** le 16 218 fusioni, cioè la CI rifiuta la misura collaborativa. Non è un difetto numerico ma la soglia $\sqrt2$ di §2.3: in zona cieca l'a priori è sostenuto da 4.5 ancore fisse in media ed è 7.3 volte migliore del ranging verso un vicino a sua volta incerto. Il guadagno che quella misura sembrava portare in Fase 4 era interamente confidenza spuria. Dettagli in [fase_5/README5.md §1.5](fase_5/README5.md).
  2. **Impianto.** La generazione della ground truth passa da `x_true(4,k+1) = v_cmd` a una legge $v^{true} = f_{slip}(v_{cmd}, x^{true}, \text{terreno})$, rappresentativa di un mezzo cingolato su fondo nevoso e basata su uno studio sulla trazione di veicoli cingolati su terreni a bassa aderenza sviluppato presso l'Università degli Studi di Trento. Il punto di innesto è già predisposto e marcato nel codice di tutte le fasi.
  3. **Modello di misura degli encoder.** È la conseguenza più critica. Il modello attuale $z_{enc} = \left[\frac{v}{r}+\frac{L\omega}{2r},\; \frac{v}{r}-\frac{L\omega}{2r}\right]^T$ presuppone rotolamento puro. In presenza di slittamento gli encoder misurano la velocità dei *cingoli*, non quella del veicolo: la relazione fra le due si carica di un errore **sistematico**, non di un rumore bianco. Poiché gli encoder sono l'osservatore primario di $v$ e $\omega$, un modello di misura polarizzato degraderebbe l'intero filtro. Occorre decidere se assorbire lo slittamento gonfiando $R_{enc}$ (soluzione minima ma statisticamente scorretta, un bias non è rumore), oppure introdurlo esplicitamente in $h(\cdot)$ tramite parametri di slittamento stimati.
  4. **Rumore di processo.** Si completa la formulazione CWNA di $Q$ (§2.5) calibrando $q_a$ e $q_\alpha$ sullo slittamento effettivamente simulato, e si attiva la legge adattiva $q_a(v) = q_{a0} + k_{terreno}\,v^2$ promessa in §1. Il parametro di terreno identificato dal D-WLS (§2.7) è l'ingresso naturale di questa taratura: la stessa struttura $\mu + c\,v^2$ che descrive la resistenza al moto governa la dipendenza dello slittamento dalla velocità, e la flotta la stima già in modo collaborativo.

  I punti 2-4 hanno anche una conseguenza sul punto 1: allargando $\Sigma$ in modo variabile nel tempo, sono la condizione in cui la soglia di accoglimento della CI può essere superata e la localizzazione collaborativa tornare a contare.
