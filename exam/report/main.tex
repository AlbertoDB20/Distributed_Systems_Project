...
effettivamente disponibili, cio\`e il GNSS se il veicolo non \`e in zona cieca
e il ricevitore ha prodotto una soluzione, una riga per ogni ancora UWB in
vista e una riga per ogni vicino in portata. Con l'EKF questo si traduce
...
$q_{lat} = 0.02$\,m$^2$/s sulla deriva laterale. Quest'ultimo \`e un termine
fondamentale: l'uniciclo \`e anolonomo e non pu\`o rappresentare la traslazione
...
di Metropolis $q_{ij}$. I due non sono tarature alternative dello stesso
parametro. La legge di controllo agisce sull'errore relativo tra i veicoli:
poich\'e opera su differenze, l'equilibrio della formazione \`e
intrinsecamente garantito per qualsiasi set di pesi $a_{ij} \ge 0$. Non vi \`e
alcuna necessit\`a di normalizzazione n\'e di definire un auto-peso $a_{ii}$.
Al contrario, i pesi $q_{ij}$ nascono per costruire una matrice stocastica (con
somma per riga pari a 1) per la stima distribuita, dove l'agente sostituisce la
propria informazione con una vera e propria media. Usarli nel controllo
significherebbe scartare l'auto-peso $q_{ii}$, distruggendo di fatto la
stocasticit\`a, e legare arbitrariamente il guadagno del controllore alla
topologia locale della rete.
...
Deve valere $d_{safe} < \min_{i \neq j}\lVert\Delta_{ij}\rVert$, altrimenti la
repulsione non si annullerebbe mai nella configurazione desiderata e la
formazione risulterebbe irraggiungibile. Inoltre $d_{safe} < r_{collab}$ \`e
...
completo che non funziona come ci si aspettava. Le fasi sono riassunte in
Tab.~\ref{tab:fasi}: il punto di arrivo di questo lavoro, e l'oggetto dei
risultati della sezione seguente, \`e la Fase 4.
...
\midrule
1 & nessuna & veicolo singolo, EKF, taratura\\
...
$C_k$ dell'EKF: la GDOP non \`e un criterio esterno importato dalla letteratura
GNSS, \`e una misura del condizionamento dello stesso problema di stima che il
filtro dovr\`a risolvere in quel punto.

L'ottimizzazione \`e affidata a Nelder-Mead, che \`e derivative-free e non
vincolato; il vincolo di installabilit\`a dentro i confini della mappa \`e
imposto come penalit\`a additiva sulla funzione di costo. Con cinque ancore lo
spazio di ricerca ha dieci dimensioni, al limite superiore di affidabilit\`a del
metodo: il risultato \`e un minimo locale dipendente dall'inizializzazione, non
l'ottimo globale.
...
Ritardo e perdita non vengono modellati separatamente ma si compongono in
un'unica grandezza, l'\textbf{et\`a} $\eta_{ij}$ del dato che il veicolo $i$ possiede di
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{figure/7_grafo_comunicazione.png}
...
\textbf{52.8\%} del tempo tutti e cinque i veicoli ricevono il GNSS, nel
\textbf{37.8\%} la copertura \`e \emph{mista} (almeno un veicolo coperto e
almeno uno al buio) e nel \textbf{9.4\%} nessun veicolo ha copertura. La fascia
centrale \`e quella per cui il progetto esiste, perch\'e \`e l\`i che un mezzo
cieco pu\`o ancorarsi a un vicino che vede ancora i satelliti; il fatto che
valga oltre un terzo della missione \`e conseguenza diretta della scala della
formazione, profonda 60\,m contro i 45--85\,m di raggio delle zone d'ombra. Con
una formazione larga pochi metri i cinque mezzi condividerebbero sempre la
stessa condizione di copertura e lo scenario collaborativo non si presenterebbe
mai.
...
\textbf{Gli Slave stimano quattro volte meglio in zona GPS-denied che a cielo
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{figure/5_copertura_e_covarianza.png}
...
lo consentirebbe senza modifiche strutturali. Il punto \`e ripreso in
Sez.~\ref{sec:conclusioni} fra i limiti noti.

I quattro Slave risultano inoltre fra loro indistinguibili, comprese le due
ali esterne che hanno un solo vicino ciascuna. Il grafo sparso
...
completa (circa 244\,000 tentativi di trasmissione).}
\label{tab:canale}
\begin{tabular}{lr}
\toprule
...
passo, che su 244\,000 tentativi si presenta comunque qualche volta. \`E il
...
guidata da un Master la rallenta quindi in proporzione diretta. La soluzione
\`e propagare $V_{rif}$ a tutta la flotta, come riportato fra gli sviluppi
futuri.
...
mai duplicarla. \`E la garanzia che i cicli del grafo non innescano
\emph{double counting}, cio\`e che nessun contributo informativo viene
conteggiato due volte mentre circola nella rete.
...
corretto non \`e il numero di condizionamento (il rapporto fra l'autovalore
massimo e quello minimo di $F$), che peggiora leggermente aggiungendo Slave pur
in presenza di maggiore informazione, ma la covarianza $(\sum_i F_i)^{-1}$, che
per l'ordinamento di Loewner pu\`o solo ridursi.
...
veri di 0.0900 e 0.00500, cio\`e $+1.44$ e $-0.44$ deviazioni standard. Come
errore relativo sul vettore dei parametri,
$\lVert \hat x_{terr} - x_{terr}\rVert / \lVert x_{terr} \rVert$, si ottiene lo
\textbf{0.31\%}; le due ali esterne, senza cooperare, si assesterebbero attorno
all'8\%. La Fig.~\ref{fig:dwls} mostra l'andamento temporale: le curve dei
cinque veicoli coincidono, mentre le punteggiate indicano cosa otterrebbe
ciascun mezzo da solo.

\begin{figure*}[t]
\centering
\includegraphics[width=0.78\textwidth]{figure/8_stima_terreno_dwls.png}
...
filtro esattamente calibrato. Il filtro risulta quindi \emph{consistente}: la
covarianza che dichiara non sottostima l'errore che commette, ed \`e la
propriet\`a che rende le sue stime utilizzabili da chi le riceve. La verifica
...
\item \textbf{Il filtro \`e ottimista sulla misura collaborativa.} La matrice
      ...
      grafo, ci\`o che un nodo emette gli ritorna dopo qualche salto e rientra
      nel suo filtro come se fosse un dato nuovo (\emph{rumor propagation}). Le
      stime diventano cos\`i correlate in modo ignoto, e imporre implicitamente
      correlazione nulla porta a sottostimare la covarianza.
      \emph{Soluzione:} la Covariance Intersection descritta pi\`u sotto.
\item \textbf{L'architettura a commutazione costa.} Il ramo UWB si attiva
      ...
      \emph{Soluzione:} sostituire la commutazione con una fusione simultanea,
      condizionando l'uso delle ancore alla loro visibilit\`a anzich\'e
      all'assenza di GNSS. L'EKF a $C_k$ di dimensione variabile la sostiene
      gi\`a senza modifiche strutturali.
...
\item \textbf{Il regressore del D-WLS \`e incerto.} La riga
      ...
      \emph{Soluzione:} una regressione a variabili strumentali, oppure la
      ripesatura di $R_i$ con la varianza di $\hat v_i$ dichiarata dall'EKF,
      che \`e gi\`a disponibile a bordo.
\item \textbf{L'aderenza \`e ideale.} La velocit\`a reale coincide istante per
      ...
      difendibile per un mezzo cingolato su neve. \emph{Soluzione:} il modello
      di slittamento descritto pi\`u sotto.
\end{itemize}
...
\`e scelto minimizzando una misura della dimensione di $\Sigma$. La
formulazione adatta al caso in esame, in cui il vicino trasmette una stima del
\emph{proprio} stato e non della grandezza cercata, \`e quella di
Carrillo-Arce \emph{et al.} \cite{carrillo2013}: l'incertezza del vicino entra
proiettata sulla direzione della congiungente. L'intervento richiede anche di
...
istantaneamente la velocit\`a comandata. Passando a un protocollo del secondo
ordine, in cui ogni agente porta nello stato di consenso sia la posizione sia la
velocit\`a e scambia entrambe, si otterrebbero tre vantaggi. Primo, il
riferimento di velocit\`a si propagherebbe attraverso la rete invece di restare
confinato al Master, eliminando il rallentamento $v_{cruise}/N$
della~\eqref{eq:velflotta}. Secondo, il Master smetterebbe di essere un punto
singolo di guasto, perch\'e ogni agente avrebbe una propria posizione desiderata
lungo il percorso. Terzo, il modello sarebbe pi\`u fedele alla dinamica reale
del mezzo, che \`e un doppio integratore e non un integratore semplice.
