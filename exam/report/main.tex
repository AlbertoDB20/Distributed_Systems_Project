...
\`e ricevibile per via di valli strette, copertura arborea e ostruzioni artificiali. 

Ciascun veicolo $i$ deve:
...
\item concorrere con gli altri all'identificazione di un parametro caratteristico del terreno,
...
\emph{percezione distribuita}: gli agenti sono mobili, ciascuno incorpora i propri
...
raggio di comunicazione $r_{collab} = 55m$.
...
superiore. \`E una scelta di progetto: rende "osservabile" quanto è informativo sulla rete un veicolo dotato di un equipaggiamento migliore in fase di cooperazione. La seconda asimmetria \`e fra ranging verso ancora fissa
...
La matrice $Q_d$ non \`e una diagonale costante ma viene ricostruita dinamicamente ad ogni
passo secondo il modello CWNA (\emph{Continuous White Noise Acceleration})
\cite{barshalom2001}. La motivazione \`e prettamente cinematica: nel modello uniciclo le coordinate $x$ e $y$ non possiedono una dinamica stocastica indipendente, ma evolvono unicamente in base alla propagazione dell'incertezza su $v$ e $\theta$. Una $Q_d$ diagonale inietterebbe rumore direttamente su $x$ e $y$,
...
$q_{lat} = 0.02$\,m$^2$/s sulla deriva laterale. Quest'ultimo \`e un termine fondamentale: l'uniciclo \`e anolonomo e non pu\`o rappresentare la traslazione
...
La formazione \`e definita da un insieme di posizioni desiderate $p_i^{des} \in \mathbb{R}^2$ e dai relativi offset $\Delta_{ij} = p_i^{des} - p_j^{des}$. La legge di controllo che governa la velocit\`a cartesiana $u_i \in \mathbb{R}^2$ comandata al veicolo $i$ \`e strutturata come un protocollo di consenso:
...
dove $K_{cons} > 0$ \`e il guadagno proporzionale [1/s], $\mathcal{N}_i$ \`e l'insieme dei vicini in portata radio e $a_{ij}$ \`e l'elemento della matrice di adiacenza. 

\`E fondamentale notare che in questa legge si utilizzano i pesi di adiacenza $a_{ij}$ e \emph{non} i pesi di Metropolis $q_{ij}$. I due non sono tarature alternative dello stesso parametro. La legge di controllo agisce sull'errore relativo tra i veicoli: poich\'e opera su differenze, l'equilibrio della formazione \`e intrinsecamente garantito per qualsiasi set di pesi $a_{ij} \ge 0$. Non vi \`e alcuna necessit\`a di normalizzazione n\'e di definire un auto-peso $a_{ii}$. Al contrario, i pesi $q_{ij}$ nascono per costruire una matrice stocastica (con somma per riga pari a 1) per la stima distribuita, dove l'agente sostituisce la propria informazione con una vera e propria media. Usarli nel controllo significherebbe scartare l'auto-peso $q_{ii}$ --- distruggendo di fatto la stocasticit\`a --- e legare arbitrariamente il guadagno del controllore alla topologia locale della rete.

Introducendo la variabile traslata $\tilde p_i = p_i - p_i^{des}$, l'errore diventa $\tilde p_i - \tilde p_j$ e l'equazione si riscrive in forma matriciale compatta come:
...
cio\`e \emph{esattamente} il protocollo di consenso lineare del corso \cite{fontanelli_notes}, dove $\otimes$ \`e il prodotto di Kronecker e $I_2$ la matrice identit\`a $2\times 2$. Il mantenimento della formazione si riduce a un problema di consenso su coordinate traslate, il cui errore decade esponenzialmente con una costante di tempo:
...
dove $\lambda_2(L)$ \`e la connettivit\`a algebrica, ossia il secondo autovalore pi\`u piccolo del Laplaciano. Questa relazione \`e stata usata come vincolo di progetto per garantire reattivit\`a uniformi. Passando dalla formazione a tre veicoli con grafo completo ($\lambda_2(L) = 3.000$) a quella a cinque veicoli con grafo sparso ($\lambda_2(L) = 0.697$), a parit\`a di guadagno il tempo di assestamento $\tau$ salirebbe da 2.22\,s a 9.56\,s. Il guadagno $K_{cons}$ \`e stato quindi ricalcolato topologia per topologia al fine di mantenere $\tau$ invariato, ottenendo per il grafo finale il valore $K_{cons} = 0.646$.

Alla stabilit\`a in tempo continuo si aggiungono due vincoli per l'implementazione in tempo discreto. Il primo riguarda l'integrazione di Eulero: la matrice di transizione di stato diventa $Q_{disc} = I - K_{cons} T_s L$, i cui autovalori sono $\lambda_{i}(Q_{disc}) = 1 - K_{cons} T_s \lambda_{i}(L)$. Per garantire la stabilit\`a asintotica deve valere $-1 < \lambda_{i}(Q_{disc}) < 1$ per ogni $i$. I parametri del progetto risultano ampiamente all'interno del limite di stabilit\`a. 
Il secondo vincolo riguarda il limite teorico di tolleranza al ritardo di comunicazione $\tau_d$ \cite{olfati2004}:
...
Per la topologia adottata, il ritardo massimo tollerabile prima che il sistema perda stabilit\`a e inizi a oscillare vale 565\,ms (più di 5 cicli).
...
dove $k_{rep}$ \`e il guadagno repulsivo ricavato invertendo un requisito di progetto che prevede che a met\`a della
distanza di sicurezza la repulsione deve valere 3\,m/s.
...
risulterebbe irraggiungibile. Inoltre, $d_{safe} < r_{collab}$, \`e
...
e costante nel tempo. Trattandosi di un parametro costante e non stocastico, la soluzione applicata \`e quella dei \emph{minimi quadrati pesati distribuiti}
(D-WLS) \cite{fontanelli_notes} siccome un filtro di Kalman distribuito servirebbe
...
\`e singolare. Diventa risolvibile solo unendo
...
dall'accumulo lungo la missione.
...
completo che non funziona come ci si aspettava. Le fasi sono riassunte in Tab.~\ref{tab:fasi}: il
...
Ogni fase \`e implementata in uno script MATLAB autonomo, che condivide con le altre un insieme di funzioni comuni (costruzione del grafo, pesi di Metropolis, rumore CWNA, consenso D-WLS). 
...
\begin{enumerate}
\item \textbf{Broadcast} --- ogni agente pubblica la propria stima dello stato sulla rete;
...
\begin{enumerate}
\item gestione \textbf{passiva}: uso il pacchetto vecchio senza applicare correzioni. Uso questa modalità con il consenso di formazione, accettando il ritardo (che si limita al massimo a 200 ms) 
\item gestione \textbf{attiva}: propago in avanti la posizione del veicolo con il modello di moto passato. Uso questa modalità con le ancore mobili UWB, stimando quindi la posizione del vicino sulla base del suo modello di moto passato.
\end{enumerate}
...
La Fig.~\ref{fig:grafo} mostra che la topologia resta deterministica per l'intera missione. Le distanze nominali si separano in due gruppi netti, 36--41\,m e 66--81\,m, con un vuoto di 25\,m in mezzo: il margine \`e del $+35\%$ sui collegamenti attivi e del $-17\%$ su quelli assenti, quindi l'errore di formazione non li fa sfarfallare.
...
guidata da un Master la rallenta quindi in proporzione diretta. La soluzione \`e di propagare $V_{rif}$ a tutta la flotta, come riportato fra gli sviluppi futuri.
...
corretto non \`e il numero di condizionamento (rapporto tra il suo autovalore massimo e il suo autovalore minimo) --- aggiungere Slave lo peggiora
